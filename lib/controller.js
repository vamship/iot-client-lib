...
Controller.prototype._createCncDispatcher = function() {
    ...
    dispatcher.requestComplete = function(message, requestId) {
        if(message instanceof Array) {
            message = _util.format.apply(_util, message);
        }
        requestId = requestId || DEFAULT_REQUEST_ID;
...
Controller.prototype._processCloudCommand = function(command) {
    ...
    switch (command.action) {
        ...
        default:
            ...
            this._cncDispatcher.requestComplete(command.requestId, [ 'Unrecognized action: [%s]', command.action ]);
            break;
    }
};
...
Controller.prototype._execStopCommand = function(category, id, requestId) {
    ...
    if(!this._isValidConnectorCategory(category)) {
        ...
        this._cncDispatcher.requestComplete(message, requestId);
...
Controller.prototype._execStartCommand = function(category, id, requestId) {
    ...
    if(!config) {
        ...
        this._cncDispatcher.warn(mssage, requestId);
        this._cncDispatcher.requestComplete(message, requestId);
...
Controller.prototype._execListConnectorsCommand = function(category, requestId) {
    ...
    this._cncDispatcher.info([ response.join('\n'), category ], requestId);
    return false;
};
...
Controller.prototype._execSendDataCommand = function(category, id, data, requestId) {
    if(!this._isValidConnectorCategory(category)) {
        this._cncDispatcher.error([ 'Invalid connector category specified: [%s::%s]', category, id ], requestId);
        this._logger.error('Invalid connector category specified: [%s]. RequestId: [%s]', category, requestId);
        return false;
    }
    var group = this._getConnectorInfo(category);
    var connectorInfo = group[id];
    if(!connectorInfo || !connectorInfo.connector) {
        this._cncDispatcher.warn([ 'Cannot send data to connector. Connector not initialized: [%s::%s]', category, id ], requestId);
...
Controller.prototype._execUpdateConfigCommand = function(category, id, config, requestId) {
    ...
    if(!config) {
        this._cncDispatcher.warn([ 'Cannot update connector config. Invalid config specified: [%s::%s]', category, id, config ], requestId);
...
Controller.prototype._execDeleteConfigCommand = function(category, id, config, requestId) {
    ...
    var config = configSection[id];
    if(!config) {
        this._cncDispatcher.warn([ 'Cannot delete connector config. No config defined: [%s::%s]', category, id ], requestId);
...
Controller.prototype._execUpdateConnectorTypeCommand = function(type, modulePath, requestId) {
    if(typeof type !== 'string' || type.length <= 0) {
        this._cncDispatcher.error([ 'Cannot update connector type. Invalid type specified: [%s].', type], requestId);
    ...
    if(typeof modulePath !== 'string' || modulePath.length <= 0) {
        ...
        this._cncDispatcher.error([ 'Cannot update connector type. Invalid path specified: [%s].', modulePath], requestId);
        return false;
    }

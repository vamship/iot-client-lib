...
Controller.prototype._initConnectorTypes = function(types) {
    ...
    this._logger.debug('Initializing connector factory. Type map:', typeMap);
...
Controller.prototype._initConnector = function(groupName, id, config) {
    ...
    connectorInfo.promise = connectorInfo.promise.then(function() {
        this._logger.info('Initializing connector: [%s]::[%s]', groupName, id);
        ...
        this._logger.info('Connector initialized: [%s]::[%s]', groupName, id);
    }.bind(this), function(err) {
        connectorInfo.actionPending = false;
        connectorInfo.result = err;
        this._logger.error('Error initializing connector: [%s]::[%s] [%s]', groupName, id, err);
        throw err;
    }.bind(this));

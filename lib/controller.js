...
Controller.prototype._processCloudCommand = function(request) {
    ...
    switch (cmd.action) {
        ...
        case MAINTENANCE_ACTION:
            this.stop(request.id).fin(function() {
...
Controller.prototype._execSendDataCommand = function(category, id, data, request) {
    ...
    // TODO: This needs to be tweaked so that CNC messages are treated differently
    // than just data dispatch messages.
    // The assumption here is that the connector will complete the request.
    connectorInfo.connector.addData(data, request);
    //request.completeOk();
    return false;
};
 ...
 * @private
 */
Controller.prototype._execDeleteConfigCommand = function(category, id, request) {
    if(!this._isValidConnectorCategory(category)) {
        this._logger.error('Invalid connector category specified: [%s]. RequestId: [%s]', category, request.id);
        request.completeError('Invalid connector category specified: [%s::%s]', category, id);
        return false;

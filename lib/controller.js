...
Controller.prototype._initConnector = function(category, id, requestId) {
    ...
    .then(function() {
        ...
        if(connectorInfo.connector) {
            var message = 'Aborting init. Connector already active: [' + category + '::' + id + ']. RequestId: [' +  requestId + ']';
            this._logger.warn(message);
            throw new Error(message);
        } else if(this._shutdownFlag) {
            var emssage = 'Aborting init. System in shutdown state: [' + category + '::' + id + ']. RequestId: [' +  requestId + ']';
            this._logger.warn(message);
            throw new Error(message);
        }
...
Controller.prototype._stopConnector = function(category, id, requestId) {
    ...
    .then(function() {
        ...
        if(connectorInfo.connector) {
            ...
            var message = 'Aborting stop. Connector not active [' + category + '::' + id + ']. RequestId: [' + requestId +']';
            this._logger.warn(message);
            throw new Error(message);
        }
    }.bind(this));
...
Controller.prototype._cloudDataHandler = function(data) {
    ...
    this._logger.debug('Processing commands from cloud', data);
...
Controller.prototype._execStopCommand = function(category, id, requestId) {
    ...
    if(!this._isValidConnectorCategory(category)) {
        this._logger.warn('Invalid connector category specified: [%s]. RequestId: [%s]', category, requestId);
        return false;
    }
    var group = this._getConnectorInfo(category);
    var connectorInfo = group[id];
...
Controller.prototype._execUpdateConfigCommand = function(category, id, config, requestId) {
    ...
    if(!config) {
        this._logger.warn('Cannot update connector config. Invalid config specified. RequestId: [%s]', requestId, config);
        return false;
    }
    var configSection = this._getConnectorConfig(category);
...
Controller.prototype._execUpdateConnectorTypeCommand = function(type, modulePath, requestId) {
    if(typeof type !== 'string' || type.length <= 0) {
        this._logger.warn('Cannot update connector type. Invalid type specified: [%s]. RequestId: [%s]', type, requestId);
        return false;
    }
    if(typeof modulePath !== 'string' || modulePath.length <= 0) {
        this._logger.warn('Cannot update connector type. Invalid path specified: [%s]. RequestId: [%s]', modulePath, requestId);
        return false;
    }

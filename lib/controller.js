...
function Controller(controllerConfig, loggerProvider) {
    ...
    this._cncDispatcher = this._createCncDispatcher();
...
/**
 * @class Controller
 * @method _getRequestCompleteSuccessHandler
 * @private
 */
Controller.prototype._getRequestCompleteSuccessHandler = function(requestId) {
    return function(data) {
        this._cncDispatcher.requestCompleteOk(null, requestId);
        return data;
    }.bind(this);
};
...
/**
 * @class Controller
 * @method _getRequestCompleteFailureHandler
 * @private
 */
Controller.prototype._getRequestCompleteFailureHandler = function(requestId) {
    return function(err) {
        this._cncDispatcher.requestCompleteError(err.toString(), requestId);
        return data;
    }.bind(this);
};
...
/**
 * @class Controller
 * @method _createCncDispatcher
 * @private
 */
Controller.prototype._createCncDispatcher = function() {
    var dispatcher = {};
    [ 'info', 'warn', 'error' ].forEach(function(methodName) {
        dispatcher[methodName] = function(message, requestId) {
    ...
    dispatcher.ackRequest = function(requestId) {
        requestId = requestId || DEFAULT_REQUEST_ID;
    ...
    dispatcher.requestCompleteOk = function(response, requestId) {
        requestId = requestId || DEFAULT_REQUEST_ID;
        if(requestId === DEFAULT_REQUEST_ID) {
            return;
        }
    ...
    dispatcher.requestCompleteError = function(message, requestId) {
        requestId = requestId || DEFAULT_REQUEST_ID;
        if(requestId === DEFAULT_REQUEST_ID) {
            return;
        }
...
Controller.prototype._initConnector = function(category, id, requestId) {
    ...
    this._cncDispatcher.info([ 'Initializing connector: [%s::%s]', category, id], requestId);
    ...
    .then(function() {
        ...
        this._cncDispatcher.info([ 'Starting connector initialization: [%s::%s]', category, id], requestId);
        var message ='';

        if(connectorInfo.connector) {
            this._cncDispatcher.warn([ 'Aborting init. Connector already active: [%s::%s]', category, id ], requestId);
            ...
            this._cncDispatcher.warn([ 'Aborting init. System in shutdown state: [%s::%s]', category, id ], requestId);
        ...
        this._cncDispatcher.info([ 'Connector initialization complete: [%s::%s]', category, id], requestId);
    }.bind(this), function(err) {
        connectorInfo.actionPending = false;
        connectorInfo.result = err;
        this._logger.error('Error initializing connector: [%s::%s]. RequestId: [%s]', category, id, requestId, err);
        this._cncDispatcher.error([ 'Error initializing connector: [%s::%s]', category, id], requestId);
        throw err;
    }.bind(this));
...
Controller.prototype._stopConnector = function(category, id, requestId) {
    ...
    .then(function() {
        ...
        this._cncDispatcher.info([ 'Stopping connector: [%s::%s]', category, id], requestId);
        connectorInfo.actionPending = true;
        ...
        if(connectorInfo.connector) {
            var promise = connectorInfo.connector.stop(requestId).fin(function() {
                ...
                this._cncDispatcher.info([ 'Connector stopped:: [%s::%s]', category, id], requestId);
                connectorInfo.actionPending = false;
                ...
                this._cncDispatcher.info([ 'Destroying connector:: [%s::%s]', category, id], requestId);
                connectorInfo.connector = null;
            }.bind(this));
            return promise;
        } else {
            this._cncDispatcher.warn([ 'Aborting stop. Connector not running [%s::%s]', category, id ], requestId);
...
Controller.prototype._processCloudCommand = function(command) {
    var arg2 = command.category || command.type || command.command;
    var arg3 = command.id || command.modulePath || 'na';

    this._cncDispatcher.info( [ 'Processing command: [%s] [%s::%s]', command.action, arg2, arg3 ], command.requestId);
    this._cncDispatcher.ackRequest(command.requestId);
    switch (command.action) {
        ...
        default:
            ...
            this._cncDispatcher.warn( [ 'Unrecognized action: [%s]', command.action ], command.requestId);
            this._cncDispatcher.requestCompleteError([ 'Unrecognized action: [%s]', command.action ],
                                                        command.requestId);
            break;
    }
};
...
Controller.prototype._execStopCommand = function(category, id, requestId) {
    ...
    if(!this._isValidConnectorCategory(category)) {
        ...
        message =  [ 'Invalid connector category specified: [%s::%s]', category, id ];
        this._cncDispatcher.error(message, requestId);
        this._cncDispatcher.requestCompleteError(message, requestId);
        return false;
    }
    var group = this._getConnectorInfo(category);
    var connectorInfo = group[id];
    if(!connectorInfo) {
        ...
        message = [ 'Cannot stop connector. Connector not initialized: [%s::%s]', category, id ];
        this._cncDispatcher.warn(message, requestId);
        this._cncDispatcher.requestCompleteError(message, requestId);
        return false;
    }
    this._stopConnector(category, id, requestId)
        .promise.then(this._getRequestCompleteSuccessHandler(requestId),
                      this._getRequestCompleteFailureHandler(requestId));
    return false;
};
...
Controller.prototype._execStartCommand = function(category, id, requestId) {
    ...
    if(!config) {
        ...
        message = [ 'Cannot start connector. No config defined: [%s::%s]', category, id ];
        this._cncDispatcher.warn(message, requestId);
        this._cncDispatcher.requestCompleteError(message, requestId);
        return false;
    };
...
Controller.prototype._execListConnectorsCommand = function(category, requestId) {
    ...
    if(typeof category === 'undefined') {
        ...
        this._cncDispatcher.error([ 'Invalid connector category specified: [%s]', category ], requestId);
        this._logger.error('Invalid connector category specified: [%s]. RequestId: [%s]', category, requestId);
        return false;
    } else {
        categories = [ category ]
    ...
    this._cncDispatcher.requestCompleteOk(response, requestId);
    return false;
};
...
Controller.prototype._execSendDataCommand = function(category, id, data, requestId) {
    ...
    if(!connectorInfo || !connectorInfo.connector) {
        ...
        message = [ 'Cannot send data to connector. Connector not initialized: [%s::%s]', category, id ];
        this._cncDispatcher.warn(message, requestId);
        this._cncDispatcher.requestCompleteError(message, requestId);
        return false;
    }
...
Controller.prototype._execUpdateConfigCommand = function(category, id, config, requestId) {
    ...
    if(!config) {
        ...
        message = [ 'Cannot update connector config. Invalid config specified: [%s::%s]', category, id, config ];
        this._cncDispatcher.warn(message, requestId);
        this._cncDispatcher.requestCompleteError(message, requestId);
        return false;
    }
    ...
    this._cncDispatcher.info([ 'Configuration updated: [%s::%s]', category, id, config ], requestId);
    this._cncDispatcher.requestCompleteOk(null, requestId);
    return true;
};
...
Controller.prototype._execDeleteConfigCommand = function(category, id, config, requestId) {
    ...
    if(!config) {
        ...
        message = [ 'Cannot delete connector config. No config defined: [%s::%s]', category, id ];
        this._cncDispatcher.warn(message, requestId);
        this._cncDispatcher.requestCompleteError(message, requestId);
        return false;
    };
    delete configSection[id];
    this._cncDispatcher.info([ 'Configuration deleted: [%s::%s]', category, id ], requestId);
    this._cncDispatcher.requestCompleteOk(null, requestId);
    return true;
};
...
Controller.prototype._execUpdateConnectorTypeCommand = function(type, modulePath, requestId) {
    ...
    if(typeof type !== 'string' || type.length <= 0) {
        ...
        message = [ 'Cannot update connector type. Invalid type specified: [%s].', type];
        this._cncDispatcher.error(message, requestId);
        this._cncDispatcher.requestCompleteError(message, requestId);
        return false;
    }
    if(typeof modulePath !== 'string' || modulePath.length <= 0) {
        ...
        message = [ 'Cannot update connector type. Invalid path specified: [%s].', modulePath];
        this._cncDispatcher.error(message, requestId);
        this._cncDispatcher.requestCompleteError(message, requestId);
        return false;
    }
    this._config.connectorTypes[type] = modulePath;
    this._cncDispatcher.info([ 'Connector type updated: [%s::%s]', type, modulePath ], requestId);
    this._cncDispatcher.requestCompleteOk(null, requestId);
    return true;
};
...
Controller.prototype._writeFile = function() {
    ...
    _fs.writeFile(this._configFilePath, JSON.stringify(this._config, null, 4), function(err) {
        if (err) {
            ...
            this._cncDispatcher.error([ 'Error writing to configuration file: [%s]', this._configFilePath, err ]);
            def.reject(err);
        } else {
            this._logger.info('Config file update successful: [%s]', this._configFilePath);
            this._cncDispatcher.info([ 'Config file update successful: [%s]', this._configFilePath ]);
            def.resolve();
        }
    }.bind(this));
    ...
    _fs.readFile(configFilePath, function(err, data) {
        ...
        if (err) {
            ...
            try {
                ...
                this._initConnectorTypes(requestId);
